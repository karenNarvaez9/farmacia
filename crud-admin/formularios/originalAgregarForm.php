...
<select class="form-select mb-3" name="codMedicamento">
  <option selected disabled>Seleccionar código medicamento</option>
 <?php 
        include("../config/conexion.php");

        $sql = $conexion->query("SELECT * FROM medicamentos");
          while ($resultado = $sql->fetch_assoc()) {
          echo "<option value='" . $resultado['codMedicamento'] . "'>" . $resultado['codMedicamento'] . "</option>";
        }
    ?>
</select>
